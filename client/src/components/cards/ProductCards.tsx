...
export const ProductCards = ({name, price, imgUrl, rating, deliveryDays, active } :  productCard) => {
    ...
    return(
    <article className="w-full h-full flex flex-col items-center gap-3 shadow-xl rounded-2xl py-5 px-5 hover:-translate-y-2 transition-[1s] border border-gray-200">
        <img src={imgUrl} alt={`Image of ${name}`} className="w-full object-cover"/>
        <h2 className="text-xl text-center flex-1">{name}</h2>
        <RateStars rate={rating} />
        <h2 className="text-center text-sm">Recíbelo a partir del <strong>{dayName}</strong>, {deliveryDate.getDay()} de {monthName}</h2>
